...
```
  distribution             nodes=30665252  refs=42565394  dangling=3766961  missing=217       unreferenced=0  root gaps=0
...
checked 42565394 references: 3766961 dangling, 217 missing node(s), 0 unreferenced node(s), 0 root gap(s), 1 store(s) affected
```
...
Unreferenced nodes are the reverse finding: records no parent names and no
version roots. They cost disk, not correctness — a prune that failed partway
leaves them, and nothing ever reads them again. Each line is the top of a
leaked subtree; what hangs below stays referenced by the garbage above it.
